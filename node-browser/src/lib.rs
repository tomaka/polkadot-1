...
use service::{
	...
	ProvideRuntimeApi, CoreApi, ParachainHost, Configuration,
};
use substrate_service::{RpcSession, Roles as ServiceRoles};
use log::{debug, info};
...
fn start_inner(wasm_ext: wasm_ext::ffi::Transport) -> Result<Client, Box<std::error::Error>> {
	...
	let chain_spec = chain_spec::ChainSpec::default().load()
		.map_err(|e| format!("{:?}", e))?;
	...
	// TODO: new_light
	let mut service = Factory::new_full(config)
		.map_err(|e| format!("{:?}", e))?;
